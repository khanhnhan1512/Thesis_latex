...
\caption{Minh họa đồ thị tri thức tĩnh.}
\label{fig:KGexample1}
\end{figure}
...
Các phương pháp LLMs cho TKGR chủ yếu dựa vào prompt đơn giản, vẫn chưa khai thác hết hiểu biết ngữ nghĩa sâu và thường bỏ qua các mối quan hệ ngữ nghĩa tiềm ẩn trong dữ liệu lịch sử. Điều này khiến mô hình khó phát hiện quan hệ gián tiếp, các khuôn mẫu phức tạp và quan hệ đa tầng giữa các sự kiện.

\subsection{Thiếu tính cá nhân hóa trong suy luận}
Phần lớn hệ thống TKGR hiện hành sử dụng chung một chiến lược cho mọi truy vấn, không phân biệt đặc thù ngữ nghĩa của từng loại. Vì vậy, các dự đoán thường chung chung, thiếu chính xác cục bộ và khó xử lý truy vấn tích hợp nhiều nguồn tri thức khác nhau.

\subsection{Khó khăn trong xử lý thông tin thời gian đa tầng}
Các phương pháp hiện tại gặp khó trong việc xử lý đồng thời:
\begin{itemize}
    \item Thông tin lịch sử bậc cao giữa các sự kiện xa nhau về thời gian.
    \item Tương tác phi tuyến ngắn hạn, dài hạn giữa các yếu tố thời gian.
    \item Xung đột tiềm ẩn giữa các nguồn tri thức khác nhau.
\end{itemize}
...
Các phương pháp TKGR hiện tại chưa cung cấp cơ chế giải thích rõ ràng, thiếu minh bạch trong quyết định, đặc biệt bất lợi trong các ứng dụng nhạy cảm và phụ thuộc nhiều vào mô hình hộp đen, khiến việc tối ưu, hiệu chỉnh hệ thống bị hạn chế.
