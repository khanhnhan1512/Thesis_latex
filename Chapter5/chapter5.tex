...
\label{Chapter5}
Chương này trình bày các thí nghiệm nhằm đánh giá hiệu quả của mô hình đề xuất. Nội dung bao gồm việc giới thiệu tập dữ liệu, các độ đo, mô hình cơ sở và kết quả so sánh hiệu suất để xác thực hiệu quả của mô hình MSKGen.
...
ngưỡng điểm để lọc ra các luật có chất lượng cao là $\gamma = 0.15$ trên cả ba tập dữ liệu.
    ...
    trong điểm số cuối cùng trên từng tập dữ liệu được thể hiện trong bảng~\ref{tab:table53_professional}.
\end{itemize}
...
\begin{table}[H]
    \centering
    \caption{Trọng số của mỗi điểm số thành phần trong điểm số cuối cùng trên từng tập dữ liệu}
    \label{tab:table53_professional}
    % 'l' căn trái cột đầu, 'c' căn giữa các cột dữ liệu
    \begin{tabular}{l ccc} 
        \toprule % Đường kẻ dày trên cùng
        & \textbf{ICEWS14} & \textbf{GDELT} & \textbf{YAGO} \\
        \midrule % Đường kẻ giữa tiêu đề và nội dung
        $\alpha$   & 0.6     & 0.6   & 0.85 \\
        $1-\alpha$ & 0.4     & 0.4   & 0.15 \\
        \bottomrule % Đường kẻ dày dưới cùng
    \end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{MSKGen w/o semantically similar facts:} Đây là biến thể của MSKGen không có giai đoạn truy xuất các sự kiện tương đồng về ngữ nghĩa.
...
\begin{figure}[ht] 
    ...
    \begin{minipage}[b]{0.48\textwidth}  
        ...
        \caption{Hit@k (k=1,3,10) của MSKGen w/o semantically similar facts với các mô hình sử dụng phương pháp suy luận dựa vào luật}
        \label{fig2: ablation study}
    \end{minipage}  
     
\end{figure} 

Quan sát kết quả từ hình~\ref{fig1: ablation study} và~\ref{fig2: ablation study}, biến thể "MSKGen w/o rule-based facts" vẫn có kết quả
...
đây sử dụng. Ngoài ra, biến thể "MSKGen w/o semantically similar facts" cũng có kết quá tốt hơn hoàn toàn so với các mô hình suy luận dựa trên luật.
...
\begin{table}[H]
    ...
    \label{tab:table57_professional}
    \begin{tabular}{l ccc} 
        \toprule
                       & \textbf{ICEWS14} & \textbf{GDELT} & \textbf{YAGO} \\
        \midrule
...
Từ bảng~\ref{tab:table57_professional}, việc thay đổi mô hình ngôn ngữ lớn sử dụng trong MSKGen không có ảnh hưởng quan trọng
